(function($) {
    ...
    var QuantityDropdown = function(input, options) {
        ...
        self.$quantity.on('click.quantity-dropdown', '.qty-dropdown .current', {
            quantityDropdown: self
        }, self.toggleDropdown);
        self.$quantity.on('click.quantity-dropdown', '.qty-options li', {
            quantityDropdown: self
        }, self.onSelect);
        self.$quantity.on('mousewheel.quantity-dropdown', '.qty-options', {
            quantityDropdown: self
        }, self.onScroll);
        self.$quantity.on('swipeup.quantity-dropdown', '.qty-options', {
            quantityDropdown: self
        }, self.onSwipeUp);
        self.$quantity.on('swipedown.quantity-dropdown', '.qty-options', {
            quantityDropdown: self
        }, self.onSwipeDown);
        if (self.$form.length) {
    ...
    QuantityDropdown.prototype.init = function() {
        ...
        self.$dropdown.append('<span class="current"><span class="value">' + self.current + '</span><span class="svg-icon icon-arrow-dropdown icon-smaller"><svg><use xlink:href="#arrow-dropdown"></use></svg></span></span>').append(self.$list).appendTo(self.$quantity);
