(function($, _) {
    var KonteProductsFilterWidget = function() {
        ...
        $('.products-filter-widget').on('input', '.products-filter__search-box', {
            widget: self
        }, self.searchTerms).on('click', '.products-filter--collapsible .products-filter__option-toggler', {
            widget: self
        }, self.toggleCollapse).on('click', '.products-filter__option-name, .products-filter__options .swatch', {
            widget: self
        }, self.toggleItem).on('change', 'input, select', {
            widget: self
        }, self.triggerItemChange).on('click', '.reset-button', {
            widget: self
        }, self.resetFilters).on('click', '.remove-filtered', {
            widget: self
        }, self.removeFiltered).on('submit', 'form.ajax-filter', {
            widget: self
        }, self.ajaxSearch);
        $(document.body).on('price_slider_create', self.addListenerToSlider).on('konte_products_filter_change', {
            widget: self
        }, self.instantSearch).on('konte_products_filter_request_success', self.updateURL).on('konte_products_filter_request_success', {
            widget: self
        }, self.updateForm).on('konte_products_filter_widget_updated', {
            widget: self
        }, self.initDropdowns).on('konte_products_filter_widget_updated', {
            widget: self
        }, self.initSliders)
    };
    ...
    KonteProductsFilterWidget.prototype.initDropdowns = function(event, form) {
        ...
        $('select', $container).each(function() {
            var $select = $(this),
                ...
                searchText = $searchBoxText.length ? $searchBoxText.text() : !1;
            $select.select2({
                dir: direction,
                width: '100%',
    ...
    KonteProductsFilterWidget.prototype.toggleCollapse = function(event) {
        ...
        $children.stop(!0, !0).slideToggle(function() {
        ...
        if ($filter.hasClass('multiple')) {
            ...
            if ($item.hasClass('selected')) {
                ...
                if ($filter.hasClass('ranges')) {
                    $input.next('input[type=hidden]').val('').prop('disabled', !0)
                }
            } else {
                $item.addClass('selected').siblings('.selected').removeClass('selected');
                $input.val(value).prop('disabled', !1);
                if ($filter.hasClass('ranges')) {
    ...
    KonteProductsFilterWidget.prototype.resetFilters = function() {
        ...
        $form.find(':input').not('[type="button"], [type="submit"], [type="reset"]').val('').trigger('change').filter('[type="hidden"]').prop('disabled', !0);
        $form.trigger('submit');
