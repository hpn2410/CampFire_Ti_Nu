jQuery(document).ready(function($) {
    ...
    $('.konte-carousel:not(.konte-carousel--elementor)').on('init', function(event, slick) {
    ...
    function initProductCarousel($carousel) {
        ...
        $products.css('opacity', 0).attr('dir', $(document.body).hasClass('rtl') ? 'rtl' : '').css('opacity', 0).on('init', function(event) {
            $(event.target).css('opacity', 1)
        }).slick(options)
    }
    var konteMansonryActive = !1;
    $('.konte-product-masonry:not(.konte-product-masonry--elementor) ul.products').on('layoutComplete', function(event) {
    ...
    $('.konte-product-tabs:not( .konte-product-tabs--elementor )').on('click', '.konte-tabs__nav li', function() {
        ...
        $.post(ajax_url, {
            ...
            $(document.body).trigger('konte_products_loaded', [$newPanel.find('li.product'), !1]);
            setTimeout(function() {
                $panels.removeClass('loading')
            }, 700)
        }).fail(function() {
    ...
    $('.konte-product-carousel2:not( .konte-product-carousel2--elementor )').each(function() {
        ...
        $carousel.find('ul.products').attr('dir', $(document.body).hasClass('rtl') ? 'rlt' : '').css('opacity', 0).on('init', function(event) {
            $(event.target).css('opacity', 1)
        }).slick(options)
    });
